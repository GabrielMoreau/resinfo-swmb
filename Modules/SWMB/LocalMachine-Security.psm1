...
Function TweakEnableBitlocker { # RESINFO
	...
	Function _EnforceCryptGPO() {
		...
		Set-ItemProperty -Path $RegPath -Name "EncryptionMethodWithXtsOs" -Value 7
		# Encryption method for fixed data drives
		Set-ItemProperty -Path $RegPath -Name "EncryptionMethodWithXtsFdv" -Value 7
		# Encryption method for removable data drives
		Set-ItemProperty -Path $RegPath -Name "EncryptionMethodWithXtsRdv" -Value 7
		...
		# Active this GPO
		Set-ItemProperty -Path $RegPath -Name "UseAdvancedStartup" -Value 1
		# Don't allow Bitlocker without TPM
		Set-ItemProperty -Path $RegPath -Name "EnableBDEWithNoTPM" -Value 0
		# Dont allow =>0, allow =>2, require=>1
		Set-ItemProperty -Path $RegPath -Name "UseTPM" -Value 2
		Set-ItemProperty -Path $RegPath -Name "UseTPMPIN" -Value 2
		Set-ItemProperty -Path $RegPath -Name "UseTPMKey" -Value 2
		Set-ItemProperty -Path $RegPath -Name "UseTPMKeyPIN" -Value 2
		...
		Set-ItemProperty -Path $RegPath -Name "DisallowStandardUserPINReset" -Value 1
		...
		Set-ItemProperty -Path $RegPath -Name "MinimumPIN" -Value 6
		...
		# Active this GPO
		Set-ItemProperty -Path $RegPath -Name "OSRecovery" -Value 1
		# Allow data recovery agent
		Set-ItemProperty -Path $RegPath -Name "OSManageDRA" -Value 1
		# Allow 48-digit recovery password
		Set-ItemProperty -Path $RegPath -Name "OSRecoveryPassword" -Value 2
		# Allow 256-bit recovery key
		Set-ItemProperty -Path $RegPath -Name "OSRecoveryKey" -Value 2
		# Omit recovery options from the BitLocker setup wizard
		Set-ItemProperty -Path $RegPath -Name "OSHideRecoveryPage" -Value 0
		# Don't Save BitLocker recovery information to AD DS for operating system drives
		Set-ItemProperty -Path $RegPath -Name "OSActiveDirectoryBackup" -Value 0
		# Store recovery passwords and key packages
		Set-ItemProperty -Path $RegPath -Name "OSActiveDirectoryInfoToStore" -Value 1
...
# 266240   0x041000   Enabled + signatures up to date
# 266256   0x041010   Enabled + signatures out of date
# 327680   0x050000   Other state/type + signatures up to date
# 331776   0x051000   Other state/type + signatures up to date
# 393216   0x060000   Disabled + signatures up to date
# 393472   0x060100   Defender Disabled
# 393488   0x060110   Disabled + signatures out of date
# 397312   0x061000   Enabled + signatures up to date
# 397568   0x061100   Defender Enabled + up to date
# 397584   0x061110   Enabled + signatures out of date
# 458752   0x070000   Other state/type
# 462848   0x071000   Enabled + signatures up to date

Function TweakViewAntivirusServices { # RESINFO
	Write-Output "Viewing Active Antivirus Services..."

	Function _GetAntivirusProductState {
		Param (
			[int]$ProductState
		)

		Switch ($ProductState) {
			262144 { Return 'Disabled' }
			262160 { Return 'OutOfDate' }
			266240 { Return 'Enabled' }
			266256 { Return 'OutOfDate' }
			393216 { Return 'Disabled' }
			393472 { Return 'Disabled' }
			393488 { Return 'OutOfDate' }
			397312 { Return 'Enabled' }
			397568 { Return 'Enabled' }
			397584 { Return 'OutOfDate' }
			462848 { Return 'Enabled' }
	...
	ForEach ($AntivirusCurrent in (Get-CimInstance -Namespace root/SecurityCenter2 -ClassName AntivirusProduct)) {
		$AntivirusName = $AntivirusCurrent.displayName `
			...
			-replace 'WithSecure™ Client Security Premium', 'WithSecure-Client-Security' `
			-replace ' ', '-'
		...
		$ServiceObject = [PSCustomObject]@{
			...
			Description = "Antivirus $($AntivirusCurrent.DisplayName) in state: $AntivirusState ($($AntivirusCurrent.productState))"
			Remediation = If ($AntivirusState -ne 'Enabled') { "Enable the antivirus $($AntivirusCurrent.DisplayName)" } Else { $Null }
		}

		If ($AntivirusState -eq 'Enabled') {
		...
		If ($AntivirusServices) {
			ForEach ($Service in $AntivirusServices) {
				...
				$ServiceObject = [PSCustomObject]@{
					...
					Description = "Antivirus $($Service.DisplayName) in state: $($Service.Status)"
	...
	If ($OneAntivirusIsUp) {
		...
		ForEach ($ServiceCurrent in $ServiceResults) {
			If ($ServiceCurrent.Status -eq 'FAIL') {
				$ServiceCurrent.Status = 'INFO'
				$ServiceCurrent.Remediation = $Null
			}
		}
	}
