...
* 2026/08/20 - `ViewScriptHost` -> Windows Script Host (execution of *.vbs scripts and alike)
...
* 2026/03/10 - `UnsetCreateTokenObject`/`SetCreateTokenObject`/`ViewCreateTokenObject` -> Unset Create a Token Object all accounts (Set use `$Global:SWMB_Custom` key `CreateTokenObject` empty by default)
...
A specific Zip archive is available on the download webpage, allowing you to distribute the `LocalMachine-SWCE.ps1` script without having to install SWMB.
...
* 2025/12/05 - `EnableAdobeEnhancedSecurity`/`DisableAdobeEnhancedSecurity`/`ViewAdobeEnhancedSecurity` -> Adobe Enhanced Security in a Standalone Application or In Browser
